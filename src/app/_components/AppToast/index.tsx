...
export const AppToast = ({
  ...
  return (
    <div className="flex align-middle">
      {icon && (
        <span className="my-auto mr-2.5 text-gray-600 flex-shrink-0">
          {icon}
        </span>
      )}

      <div className="flex items-center justify-between gap-4">
        <div className="my-auto w-full text-black text-xs font-semibold 640:text-sm">
          {children}
        </div>
        <ActionButton
...
const ActionButton = ({ onClick, actionType }: ActionButtonProps) => (
  <button
    {...{ onClick }}
    className="my-auto -mr-2 h-full p-2 transition-colors bg-white rounded-full cursor-pointer"
  >
    {actionType === "close" ? (
      <svg
