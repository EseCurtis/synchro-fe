...
const DeclineService = ({ service, onClose }: any) => {
  ...
  return (
    <div>
      <h3 className="font-bold flex gap-2">{declineIcon} Decline Service</h3>
      <div className="form items-left mt-4">
        <p>
          Are you sure you want to decline <b>{service?.name}?</b>
        </p>
      </div>
      <Input
      ...
      <div className="mt-5 flex gap-4 items-center">
        <Button
          ...
          onClick={() => {
            mutate({ eventId: service?.id, status: "rejected", reason });
          }}
        >
          Decline {isLoading && <Spinner />}
        </Button>
